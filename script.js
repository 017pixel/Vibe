document.addEventListener('DOMContentLoaded', () => {
    ...
    const I18N = {
        de: {
            ...
            settings_close: 'Schließen', focus_start: 'Start', focus_stop: 'Stopp',
            focus_pomodoro: 'Pomodoro', focus_stopwatch: 'Stopuhr',
            shop_buy: 'Kaufen', shop_unlocked: 'Frei', stat_streak: 'Streak', stat_streak_days: 'Tage',
            stat_total: 'Sitzungen gesamt', stat_weekly: 'Minuten diese Woche',
        ...
        en: {
            ...
            settings_close: 'Close', focus_start: 'Start', focus_stop: 'Stop',
            focus_pomodoro: 'Pomodoro', focus_stopwatch: 'Stopwatch',
            shop_buy: 'Buy', shop_unlocked: 'Owned', stat_streak: 'Streak', stat_streak_days: 'Days',
            stat_total: 'Total Sessions', stat_weekly: 'Minutes this week',
    ...
    const streakCountEl = document.getElementById('streak-count'); // Hinzugefügt
    ...
    async function loadData() {
        try {
            ...
            if (savedState) {
                state = { ...defaultState, ...savedState, userData: {...defaultState.userData, ...savedState.userData}, settings: {...defaultState.settings, ...savedState.settings} };
    ...
    // NEU: Überprüft beim App-Start, ob der Streak gebrochen wurde.
    ...
    // ÜBERARBEITET: Aktualisiert den Streak nach einer erfolgreichen Session.
    function updateStreak() {
        ...
        // Setze das Datum der letzten Sitzung auf heute, da gerade eine abgeschlossen wurde.
        state.userData.lastSessionDate = today.toISOString();

        if (!lastSession) {
            // Die allererste Sitzung.
            state.userData.streak = 1;
            return;
        }
        
        if (lastSession.getTime() === today.getTime()) {
            // Weitere Sitzung am selben Tag. Streak ändert sich nicht, aber falls er 0 war, wird er 1.
        ...
        if (diffDays === 1) {
            // Sitzung am Folgetag -> Streak erhöhen.
            state.userData.streak += 1;
        } else if (diffDays > 1) {
            // Streak war gebrochen -> Neuer Streak beginnt bei 1.
            state.userData.streak = 1;
        }
    }
    ...
    // --- UI RENDERING ---
    // ÜBERARBEITET: Aktualisiert jetzt auch die Streak-Anzeige
    function updateUI() {
        coinCountEl.textContent = state.userData.coins;
        streakCountEl.textContent = state.userData.streak;
        streakCountEl.classList.toggle('active-streak', state.userData.streak > 0);
    ...
    function renderStatsScreen() {
        ...
        const dayLabels = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'].map(d => I18N[state.settings.language][`stat_day_${d}`]);
        ...
        state.sessions.forEach(session => {
            ...
            if (sessionDate >= startOfWeek) {
                weeklyMinutes[sessionDate.getDay()] += session.duration;
            }
        });
            ...
            <div class="stat-card">
                ...
                <div class="chart-container">
                    ${weeklyMinutes.map((mins, i) => `
                        <div class="chart-bar" style="height: ${ (mins / maxWeeklyMinutes) * 100}%" title="${dayLabels[i]}: ${mins} min">
    ...
    closeSettingsButton.addEventListener('click', () => toggleSettingsSheet(false));
    ...
    // --- INITIALIZATION ---
    // ÜBERARBEITET: Ruft checkStreak() beim Start auf.
    async function init() {
        try {
            await db.init();
            await loadData();
            checkStreak(); // Prüfe den Streak-Status, bevor die UI angezeigt wird.
            updateUI();
        } catch (error) {
